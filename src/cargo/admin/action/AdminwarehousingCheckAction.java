...
public class AdminwarehousingCheckAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		map = dao.getInfo(request.getParameter("state"), map.get("num").toString());
		...
		forward.setPath("../admin/checkBeforeItems.jsp");
		return forward;
	}
	
	private String makeID(String firstSixofID){
		//item id들과 중복 비교 하기 위해 앞에 "house+날짜" 형식이 같은 id 가져오기
		ArrayList<String> IDs = dao.getItemIDs(firstSixofID);
